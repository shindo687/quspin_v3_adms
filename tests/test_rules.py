...
ORACLE_STEPS = (1e-4, 3e-5, 1e-5, 3e-6, 1e-6)


def central(fun, x, dx, eps):
    return (fun(x + eps * dx) - fun(x - eps * dx)) / (2 * eps)


def central_scan(fun, x, dx):
    """Evaluate a central-difference oracle at all documented steps."""
    return [central(fun, x, dx, eps) for eps in ORACLE_STEPS]
...
def test_kl_primal_jvp_vjp_and_duality() -> None:
    ...
    oracle_scan = [
        ...
        for left, right in zip(
            central_scan(lambda x: KL_div(x, p2), p1, dp1),
            central_scan(lambda x: KL_div(p1, x), p2, dp2),
        )
    ]
    assert value == KL_div(p1, p2)
    for oracle in oracle_scan:
...
def test_coherent_state_real_and_complex_oracle() -> None:
    ...
    for oracle in central_scan(
        lambda x: native_coherent_state(x, 6, dtype=np.complex128), a, da
    ):
    ...
    with pytest.raises((TypeError, ValueError), match="scalar tangent"):
        ad.jvp(
            native_coherent_state,
            0.8,
            4,
            tangents={"a": np.ones((1, 1))},
        )
...
def test_matrix_rules_oracle_and_duality() -> None:
    ...
    for function in (commutator, anti_commutator):
        ...
        oracle_scan = [
            left + right
            for left, right in zip(
                central_scan(lambda x: function(x, h2), h1, d1),
                central_scan(lambda x: function(h1, x), h2, d2),
            )
        ]
...
def test_project_op_dense_structured_rule() -> None:
    ...
    for eps in ORACLE_STEPS:
        ...
        oracle = (plus["Proj_Obs"] - minus["Proj_Obs"]) / (2 * eps)
        assert np.allclose(tangent["Proj_Obs"], oracle, rtol=2e-6, atol=2e-6)
